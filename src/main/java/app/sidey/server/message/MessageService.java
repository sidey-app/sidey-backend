...
public class MessageService {
    ...
    public MessageService(DSLContext db,Transactions tx,AuthService auth,MeterRegistry metrics){this.db=db;this.tx=tx;this.auth=auth;this.metrics=metrics;}
    ...
    public Message send(UUID user,UUID room,UUID id,String body) {
        ...
        try {
            Message result=tx.run(()->{
                ...
                return map(db.fetchOne("insert into messages(id,room_id,sender_id,body,bubble_style_id,created_at) values (?,?,?,?,?,clock_timestamp()) returning *",id,room,user,normalized,bubble));
            });
    ...
    public Page history(UUID actor,UUID room,Cursor after,Cursor before,int limit){return tx.run(()->{
        require(actor,room);
